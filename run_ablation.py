...
def run_simulation(mode='baseline'):
    ...
    while not done:
        #dinoume to observation, gientai return to action(dhladh ti prepei na kanei to HEV)
        action, _ = model.predict(obs)
        
        
        if mode == 'neuro_symbolic':
            #LLM/Rule-Based Logic
            #katw apo 20 km/h eimaste se EV Mode
            current_speed = obs[0] #speed prwto stoixeio tou observation
            
            if current_speed < 20.0:
                #edw einai pou to LLM "diorthwnei" to PPO, giati afto den katalavainei oti an paem katv apo 20 prepei na svhsei th mhxanh
...
def run_study():
    ...
    #Sygkrish kai dhmiourgia report
